...
A stair-step pattern is visible in both work-efficient implementations because non-power-of-two inputs are padded to the next power of two before the scan. For example, inputs between 3M and 4M elements are padded to 2^22 elements, while inputs between 5M and 8M are padded to 2^23. Since inputs within each range operate on the same padded array size, they perform nearly the same amount of instructions, which explains the relatively flat runtime within each range and the sudden increases when the next power-of-two boundary is crossed.
